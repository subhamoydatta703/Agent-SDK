...
for (const q of ['What is (5+3)*2?', 'What is 123*456?', 'What is sqrt(81)+max(2,7)^2?']) {
...
`The result is 16.`, `The result is 56088.`, `The result is 58.`

## How the smart default works

- If the last message is a **tool result**, it answers with the returned value.
- If the last message is a **user question**, it extracts the math expression, validates it
  with the same recursive-descent parser used by `calculatorTool` (`safeEval`), and emits a
